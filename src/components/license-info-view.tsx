...
export function formatActivation(iso: string) {
...
function remaining(used: number, quota: number) {
...
export function LicenseInfoView({
  ...
  const requestId = revealSecrets ? license.requestId : display.requestId;
  const hardwareId = revealSecrets ? license.hardwareId : display.hardwareId;
  const softwareKey = revealSecrets ? license.softwareKey : display.softwareKey;

  return (
    <>
      <Panel title="Informasi Lisensi">
        <LicenseRow icon={<Mail className="size-4" />} label="Email Terdaftar" value={license.email} />
        <LicenseRow
          ...
          zebra
        />
        <LicenseRow icon={<Cog className="size-4" />} label="Request ID" value={requestId} />
        <LicenseRow
          ...
          zebra
        />
        <LicenseRow icon={<CreditCard className="size-4" />} label="Software Key" value={softwareKey} />
      </Panel>
      ...
      <Panel className="mt-4" title="Informasi Layanan">
        ...
        <QuotaRow
          ...
          detail={`${services.mikrotik.used} Router | ${remaining(services.mikrotik.used, services.mikrotik.quota)} Quota`}
        ...
        <QuotaRow
          ...
          detail={`${services.session.used} | ${remaining(services.session.used, services.session.quota)} Quota`}
        ...
        <QuotaRow
          ...
          detail={`${services.pelanggan.used} Quota`}
