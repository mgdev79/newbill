...
export default function RadiusEnginePage() {
  ...
  async function load() {
    const data = await fetch("/api/v1/saas/radius-engine").then((r) => r.json());
    setRows(data.rows ?? []);
  }
  ...
  async function save() {
    ...
    const response = await fetch(
      editingId ? `/api/v1/saas/radius-engine/${editingId}` : "/api/v1/saas/radius-engine",
      {
  ...
  async function remove(id: string, name: string) {
    ...
    setBusyId(id);
    await fetch(`/api/v1/saas/radius-engine/${id}`, { method: "DELETE" });
    setBusyId(null);
  ...
  async function testForm() {
    ...
    const endpoint =
      editingId && !form.dbPassword
        ? `/api/v1/saas/radius-engine/${editingId}/test`
        : "/api/v1/saas/radius-engine/test";
  ...
  async function testRow(row: Engine) {
    setBusyId(row.id);
    const response = await fetch(`/api/v1/saas/radius-engine/${row.id}/test`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
